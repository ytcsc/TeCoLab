...
\begin{itemize}
    ...
    \item \textbf{Python 3 dependencies:} Refer to Section~\ref{sec:dependencies};
...
\section{Configuring a Python Environment}\label{sec:dependencies}

Before executing your TeCoLab experiment, you need to configure a Python virtual environment. Open a terminal, navigate to the folder containing \emph{TeCoLab.py}, and run the following command:

\begin{lstlisting}[language=bash]
>> python3 -m venv TCLenv
\end{lstlisting}

This will create a folder named \texttt{TCLenv} (short for \emph{TeCoLab environment}), which allows you to manage project-specific dependencies without affecting your global Python setup. Once the environment is created, activate it with the appropriate command for your operating system:
...
Replace \texttt{XXX} with the desired period in milliseconds. For example, setting \texttt{XXX} to 500 means that an experiment iteration will occur every 500 ms, instead of the default 200 ms. Adjusting this period can be beneficial if your control algorithm's computation takes more than the default 200 ms. However, be cautious, as changing this value will also affect the period of control algorithms.
...
Ideally, the time values in the log file should be evenly spaced, with a default spacing of 200 ms (see Section~\ref{sec:TeCoLabPeriod}). However, some variation (around 10 ms) is not uncommon. If the variation is excessive, potential causes include:
\begin{itemize}
